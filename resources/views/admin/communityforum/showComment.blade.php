            ...
            @if(is_iterable($comments) && count($comments) > 0)
                @forelse ($comments as $comment)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('admin.updateComment', [$communityforums->id, $comment->id]) }}" class="btn btn-warning">Edit Comment</a>
                            <form method="post" action="{{ route('admin.deleteComment', [$communityforums->id, $communityforum->id]) }}" style="display: inline;">
                                @csrf
                                @method('DELETE')
